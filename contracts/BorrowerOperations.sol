...
contract BorrowerOperations is LiquityBase, Ownable, CheckContract, IBorrowerOperations {
    ...
    function openTrove(uint _maxFeePercentage, uint _LUSDAmount, address _upperHint, address _lowerHint) external payable override {
        ...
        // check if TCR < CCR (it's 150%)
        bool isRecoveryMode = _checkRecoveryMode(vars.price);
        // Valid fee percentage
        _requireValidMaxFeePercentage(_maxFeePercentage, isRecoveryMode);
        // Check trove status. require it not active
        ...
        if (isRecoveryMode) {
            ...
            // TCR >= CCR (150%) if not it is recovery mode
            _requireNewTCRisAboveCCR(newTCR);
        }

        // Set the trove struct's properties
        // create/update trove with status = active
        contractsCache.troveManager.setTroveStatus(msg.sender, 1);
        // increase collateral value (ETH)
        contractsCache.troveManager.increaseTroveColl(msg.sender, msg.value);
        // increase compositeDebt
        contractsCache.troveManager.increaseTroveDebt(msg.sender, vars.compositeDebt);
        // require borrower operators
        ...
        // send LUSD debt to sender
        // increase netDebt to activePool
        ...
        // send LUSD_GAS_COMPENSATION to gasPoolAddress
        // increase LUSD_GAS_COMPENSATION to activePool
    ...
    function addColl(address _upperHint, address _lowerHint) external payable override {
        // address _borrower, uint _collWithdrawal, uint _LUSDChange, bool _isDebtIncrease, address _upperHint, address _lowerHint, uint _maxFeePercentage
    ...
    */
    ...
    function closeTrove() external override {
        ...
        IRUSDToken lusdTokenCached = lusdToken;
        ...
        // check valid sender's LUSD balance: balanceOf(sender) > debtAmount - LUSD_GAS_COMPENSATION(200LUSD)
    ...
    function _triggerBorrowingFee(ITroveManager _troveManager, IRUSDToken _lusdToken, uint _LUSDAmount, uint _maxFeePercentage) internal returns (uint) {
    ...
    function _withdrawLUSD(IActivePool _activePool, IRUSDToken _lusdToken, address _account, uint _LUSDAmount, uint _netDebtIncrease) internal {
    ...
    function _repayLUSD(IActivePool _activePool, IRUSDToken _lusdToken, address _account, uint _LUSD) internal {
    ...
    function _requireSufficientLUSDBalance(IRUSDToken _lusdToken, address _borrower, uint _debtRepayment) internal view {
